...
namespace Exiled.Events.Patches.Events.Player
{
    ...
    internal static class PreAuthenticating
    {
        ...
        public static int authenticatePlayer(string userId, ConnectionRequest request, byte flags, string country, int cur_slots)
        {
            ...
            {
                Exiled.API.Features.Log.Info($"What is current peer count and such {LiteNetLib4MirrorCore.Host.ConnectedPeersCount} and {cur_slots} and max {LiteNetLib4MirrorNetworkManager.singleton.maxConnections}" +
                                             $" flags {flags} and country {country}");

                API.Features.Log.Info("Accepted");
                return 2;
            }
